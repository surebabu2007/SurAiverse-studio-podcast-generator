...
# Paralinguistic tags supported
PARALINGUISTIC_TAGS = [
    "[laugh]", "[chuckle]", "[gasp]", "[sniff]", "[groan]", 
    "[cough]", "[shush]", "[sigh]", "[clear throat]"
]

# Average speaking rate: words per minute (adjustable)
AVERAGE_WPM = 150  # Average human speaking rate
...
class GeminiClient:
    ...
    def _inject_natural_paralinguistic_tags(self, content: str, min_tags: int = 3) -> str:
        ...
        # but only between paragraphs, not mid-sentence
        if tags_added < tags_needed:
            paragraphs = modified_content.split('\n\n')
            if len(paragraphs) >= 3:
                # Add at most one tag at a natural paragraph transition
...
Enhanced text with tags:"""
        ...
        try:
            ...
            print(f"[Enhance] Failed to enhance text: {e}")
            return text
